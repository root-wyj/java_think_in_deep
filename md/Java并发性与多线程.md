...
`final`关键字在线程安全性中也有非常重要的作用。

从本质上讲，final能够做出以下保证：**当你创建一个对象时，使用final关键字能使另外一个线程不会访问到处于“部分创建”的对象。**因为final能够保证以下语义：

> **当构造函数结束时，final类型的值被保证其他线程访问对象是，他们的值是可见的。**

`final`是所谓`安全发布（safe publication）`的一种方式。在一个线程中创建完对象，另外一个线程通过引用访问这个对象的成员变量的时候，final关键字能够保证被final修饰的成员变量是一定赋完值的。如果没有final关键字，Java内存模型是存在重排序的情况的，就是说当栈中的引用指向堆中对象的地址的时候，也就是说其他线程获取到对象的引用的时候，也就是说本线程导出对象的引用的时候，对象的成员变量可能并没有赋值完成，在多线程的环境下，访问成员变量是时时刻刻都可能发生的，所以也就存在其他线程访问到了并没有初始化（而仅仅是零值）的成员变量，最后导致了程序的不正确运行。

而`final`关键字可以防止此类事情的发生。**如果某个成员是final的，JVM规范做出如下保证：一旦对象引用对其他线程可见，其final成员也必须正确赋值了。**

需要注意的是，final修饰的成员变量虽然不可变，但是如果是引用类型仅仅能保证指向的引用地址不会变，但是并不能保证该引用类型中的成员变量不会变，如果想要一个final修饰的引用类型也保证不可变，就要求该引用类型的成员变量都是final修饰的，并且要么是基本类型，要么指向另一个不可变的对象。
